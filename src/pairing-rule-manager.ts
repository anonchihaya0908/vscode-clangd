...
export async function writeRules(rules: PairingRule[],
  ...
  try {
    ...
    const target = scope === 'workspace' ? vscode.ConfigurationTarget.Workspace
                                         : vscode.ConfigurationTarget.Global;
    await vscode.workspace.getConfiguration('clangd').update(CONFIG_KEY, rules,
                                                             target);
  } catch (error) {
...
export async function resetRules(scope: 'workspace'|'user'): Promise<void> {
  try {
    const target = scope === 'workspace' ? vscode.ConfigurationTarget.Workspace
                                         : vscode.ConfigurationTarget.Global;
    await vscode.workspace.getConfiguration('clangd').update(CONFIG_KEY,
                                                             undefined, target);
  } catch (error) {
...
function createRuleChoices(): RuleQuickPickItem[] {
  return EXTENSION_OPTIONS.map(
      (option, index) => ({
        label: `$(file-code) ${option.label}`,
        description: option.description,
        rule: {
          key: `custom_${option.language}_${index}`,
          label: `${option.language.toUpperCase()} Pair (${option.headerExt}/${
              option.sourceExt})`,
...
async function handleRuleSelection(rule: PairingRule): Promise<void> {
  ...
  await PairingRuleService.writeRules([rule],
                                      selection.scope as 'workspace' | 'user');
...
export async function showAdvancedManagementMenu(): Promise<void> {
  const selection = await vscode.window.showQuickPick(
      createAdvancedMenuItems(), {title: 'Advanced Rule Management'});
