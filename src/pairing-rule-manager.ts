...
export class PairingRuleService {
  ...
  private static handleError(error: unknown, operation: string, scope: string): never {
  ...
  public static getActiveRules(): ReadonlyArray<PairingRule> {
    return vscode.workspace
      ...
      .get<PairingRule[]>(PairingRuleService.CONFIG_KEY, []);
  }

  public static hasCustomRules(scope: 'workspace' | 'user'): boolean {
    const inspection = vscode.workspace
      .getConfiguration('clangd')
      .inspect<PairingRule[]>(PairingRuleService.CONFIG_KEY);
    const value =
      scope === 'workspace' ? inspection?.workspaceValue : inspection?.globalValue;
  ...
  public static async writeRules(
    rules: PairingRule[],
    ...
    try {
      ...
      await vscode.workspace
        .getConfiguration('clangd')
        .update(PairingRuleService.CONFIG_KEY, rules, target);
    } catch (error) {
  ...
  public static async resetRules(scope: 'workspace' | 'user'): Promise<void> {
    try {
      ...
      await vscode.workspace
        .getConfiguration('clangd')
        .update(PairingRuleService.CONFIG_KEY, undefined, target);
    } catch (error) {
...
export class PairingRuleUI {
  ...
  public static async showConfigurationWizard(): Promise<void> {
    ...
    quickPick.items = [...PairingRuleUI.createRuleChoices(), ...PairingRuleUI.createAdvancedOptions()];
